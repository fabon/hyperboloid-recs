...
class HyperboloidDistance(Function):
    def minkowski_tensor_dot(u, v):
        temp=np.eye(global_variables.N_DIM)
        temp[0, 0] = -1.
        if u.device.type == 'cuda':
            # T = th.FloatTensor(temp).cuda()
            T = th.FloatTensor(temp).cuda(u.device.index)
        else:
            T = th.FloatTensor(temp)
        v_neg = th.matmul(v, T)
        obj_prev=th.mul(u, v_neg)
        obj=th.sum(obj_prev, -1, keepdim=True)  # keep dims for broadcasting
        return obj

    def forward(self, u, v):
        self.save_for_backward(u, v)

        x=-HyperboloidDistance.minkowski_tensor_dot(u, v)
        z = th.sqrt(th.clamp(th.pow(x, 2) - 1, min=0))
        res=th.log(th.clamp(x + z, min=1)).squeeze(-1)
    ...
    def backward(self, g):
        ...
        g = g.unsqueeze(-1)
        coeff = 1./(th.sqrt(th.clamp(HyperboloidDistance.minkowski_tensor_dot(u, v)**2-1.,min=0))+global_variables.MY_EPS)
        gu = coeff*v
        gv = coeff*u
        chained_gu=g.expand_as(gu) * gu
        chained_gv=g.expand_as(gv) * gv
        return chained_gu, chained_gv
...
class CosineDistance(nn.Module):
    ...
    def forward(self, u, v):
        w_u = u.norm(p=2, dim=1, keepdim=True)
        w_v = v.norm(p=2, dim=1, keepdim=True)
        u_normalised=u/w_u
        v_normalised=v/w_v
        return th.sum(th.pow(u_normalised - v_normalised, 2), dim=-1)
